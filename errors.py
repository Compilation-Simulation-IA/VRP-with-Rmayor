...
class CompilerError(RmayorError):
...
class LexicographicError(RmayorError):
...
class SyntaticError(RmayorError):
    'Errores detectados en el rmayor_parser'
...
class SemanticError(RmayorError):
...
class TypesError(SemanticError):
    ...
    INCONSISTENT_ARG_TYPE = 'In call of method %s, type %s of parameter %s does not conform to declared type %s.'
