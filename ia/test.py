...
def raise_frame(frame, init=False, update_target=False, target_entry=None, f_thres_slider=None):
    ...
    if init:
        population = search1.init_population(max_population, gene_pool, len(target))
...
def genetic_algorithm_stepwise(population):
    ...
    for generation in range(ngen):
        ...
        population = [
            search1.mutate(search1.recombine(*search1.select(2, population, fitness_fn)), gene_pool, mutation_rate) for i
        ...
        fittest_individual = search1.fitness_threshold(fitness_fn, f_thres, population)
